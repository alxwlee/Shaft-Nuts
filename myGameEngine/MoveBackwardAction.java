...
public class MoveBackwardAction extends AbstractInputAction {
	...
	public MoveBackwardAction(MyGame g, ProtocolClient p, Node n ) {
		game = g;
		protClient = p;
		avN= n;
	}
	...
	public void performAction(float time, Event event) {
		...
		{
			game.getEngine().getSceneManager().getSceneNode("myDolphinNode").moveBackward(game.getMoveSpeed());
				...
				game.getEngine().getSceneManager().getSceneNode("myDolphin2Node").getLocalPosition()) <= 10)
		{
			...
			//game.getEngine().getSceneManager().getSceneNode("cameraPositionNode").moveBackward(game.getMoveSpeed());	
			
			//game.camera.setPo((Vector3f)game.getEngine().getSceneManager().getSceneNode("cameraPositionNode").getLocalPosition());
			
		}
		*/
		avN.moveBackward(0.25f);
